...
;smoothsig=1d0
template=temp
template=gauss_smooth(temp, smoothsig)
;stop
x=dindgen(n_elements(temp))
temp_depth=poly(x, temp_depth_coeff)

template=template^temp_depth
...
temp_over=interpol(template, wl_soln, wl_soln_over_full)

wl_shifted=wl_soln_over_full*(1d0+(delta_rv/bigc))
;stop
star_model=interpol(temp_over, wl_shifted, wl_soln_over_full)
...
if (visualize eq 1) then begin
    ...
    title_str="RV shift: "+strtrim(delta_rv,2)+" | visit: " + strtrim(visit,2)+" | smooth: " + strtrim(smoothsig,2) +" | depth: " + strtrim(temp_depth_coeff,2) + " | chi2: "+strtrim(chi2,2)
;    title_str="RV shift: "+strtrim(delta_rv,2)+" | visit: " + strtrim(visit,2)+" | depth: " + strtrim(temp_depth_coeff,2) + " | chi2: "+strtrim(chi2,2)

    plot, wl_soln, int_obs_dim,yr=[0.1,1.1], /xs, title=title_str,xtitle="Wavelength (microns)", charsize=1.0
...
guess=[rv_guess, depth_guess, smooth_guess] 
scale=[rv_scale, depth_scale, smooth_scale]
