...
pro ircsrv_allchunks, epoch=epoch, object=object, trace=trace, visualize=visualize, first_pix=first_pix, npix_select=npix_select, mode=mode, run=run, n_bases_lsf=n_bases_lsf, n_other=n_other, s_iter=s_iter, current_tag=current_tag, initial_file=initial_file, s_template_num=s_template_num, n_lsf_ga=n_lsf_ga, output_file=output_file, _extra=ex
...
if n_elements(ex.visit) eq 0 then visit=0 else visit=ex.visit
...
;   endelse
  ...
  ;;; --- ALL TOGETHER NOW ---
            ...
            if fmode eq 'amoeba' then begin
                ...
                guess=[rv_guess,0.5d0,0.5d0,0d0,1.5d0,0.9d0,1d0]
                scale=[0.4d0,0.5d0,0.5d0,1d-5,1d0,0.1d0,0.1d0]
