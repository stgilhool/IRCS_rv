...
pro ircsrv_rvaug3, epoch=epoch, object=object, trace=trace, visualize=visualize, first_pix=first_pix, npix_select=npix_select, fmode=fmode, run=run, n_bases_lsf=n_bases_lsf, n_other=n_other, s_iter=s_iter, current_tag=current_tag, initial_file=initial_file, s_template_num=s_template_num, n_lsf_ga=n_lsf_ga, output_file=output_file, visit=visit, model_tag=model_tag
...
parind=parinfo_getindex(model_par, /all)
wl_coeff=model_par[parind.wli].tbestfit
wl_scale=(model_par[parind.wli].tlimits[1]-model_par[parind.wli].tlimits[0])/2
...
;bcv0=params1[0]
bcv0=params1[0] ;exp 0 is excluded, so now it's all relative to exp 1
...
;    struct=mrdfits(ABobj_file[visit], 1)
 ;   head=struct.header
    ...
    if file_test(input_file) then begin    
        ...
        if strmatch(input_file, '*best.fits') eq 1 then begin

            ;info=mrdfits(input_file,1)
            info=mrdfits(input_file,2)

            ind=parinfo_getindex(info, /all)
            gh0_guess=info[ind.ghi[0:n_bases_lsf-1]].tbestfit
            sig_start=gh0_guess[0]
            wl_start=1d-12
            
            ;h2o_depth_guess=info[ind.ti[1]].tbestfit
            ;co2ch4_depth_guess=info[ind.ti[2]].tbestfit
            ;nh3_depth_guess=info[ind.ti[0]].tbestfit
