...
;;;;;MAIN BODY;;;;;
;;;;;;;;;;;;;;;;;;;
pro IRCS_cal_lsftest, wl_guess, wl_scale, gh_guess, gh_scale, gh_lin_guess, gh_lin_scale, other_guess, other_scale, run, niter, lin_switch=lin_switch, min_type=min_type, visualize=visualize, oversamp=oversamp, npix_trim_start=npix_trim_start, npix_trim_end=npix_trim_end, model_func=model_func
...
pi = replicate({fixed:0, limited:[0,0], limits:[0.D,0.D]},n_elements(guess))
;fix gh0
pi[n_elements(wl_guess)].fixed=1
;fix gh
pi[(nparam_wl+nparam_gh)].fixed=1
...
if model_func ne 0 then begin
    ...
    if min_type eq 'amoeba' then begin
        ...
        r=mpfit('amoebafunction', guess, bestnorm=chi2, ftol=ftol, maxiter=1000L, parinfo=pi, /quiet)
    endif
endelse
...
outpath_ext='lsf_test_nosigchange/'
...
openw, lun, 'lsf_test_nosigchange.txt', /get_lun
printf, lun, gh_guess[0], chi2perDOF, gh_result[0], gh_lin_result[0]
close, lun
free_lun, lun
