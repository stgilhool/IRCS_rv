function ircsrv_lsf, gh0_vector, gh1_vec=gh1_vec, oversamp=oversamp, first_pix=first_pix, npix_select=npix_select, verbose=verbose, decomp=decomp, neg_penalty=neg_penalty, neg_boost=neg_boost

;Idea for improvement: pass H_coeff in with function call?
npix=1024L ;CAUTION this needs to be tuned for non-IRCS data sets

;Deal with inputs
nparam_gh=n_elements(gh0_vector)
;Set verbose
if n_elements(verbose) eq 0 then verbose=0
;Set decomp
if n_elements(decomp) eq 0 then decomp=0
;Set negative lsf options
; *** neg_boost
if n_elements(neg_boost) eq 0 then neg_penalty_switch = 1 else neg_penalty_switch=0
...
for basis=0, nparam_gh-1 do begin
    ...
    gh_cube[*,*,basis]=basis_coeff[*,*,basis]*gh_cube_nonorm[*,*,basis]
endfor
...
if neg_penalty_switch eq 0 then begin
...
;;;;
lsf_com=rebin(reform(total(abs(lsf)*x_sig_2d, 1, /double), 1, npix_model), npix_lsf, npix_model)
...
;    norm_com=rebin(reform(total(abs(gh_cube_nonorm_com[*,*,basis]), 1, /double), 1, npix_model), npix_lsf, npix_model)
 ;   gh_cube_norm_com[*,*,basis]=gh_cube_nonorm_com[*,*,basis]/norm_com
    ...
    gh_cube_com[*,*,basis]=basis_coeff[*,*,basis]*gh_cube_nonorm_com[*,*,basis]
endfor
...
;Include the negativity penalty
if neg_penalty_switch eq 1 then begin
    abs_norm=total(abs(lsf_com), 1, /double)
    diff_norm=total(lsf_com, 1, /double)
    twodiff=abs_norm-diff_norm
    ;neg_penalty=total(twodiff, /double)/(2d0*oversamp)
    neg_penalty=total(twodiff, /double)
endif

;Decompose the LSF, if necessary
if decomp then begin
    if neg_penalty_switch eq 0 and neg_count_com gt 0 then begin
