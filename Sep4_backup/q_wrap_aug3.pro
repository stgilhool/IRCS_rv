pro q_wrap_aug3, first_first_pix=first_first_pix, pix_step=pix_step, model_out_tag=model_out_tag, _EXTRA=ex2
...
for visit=0,12 do begin


    output_file=output_base+'_' +strtrim(visit+1,2)+'.fits'
    for i=0,3 do begin
    
        first_pix=first_first_pix + i*pix_step
        
        ircsrv_rvaug3, first_pix=first_pix, visit=visit+1, output_file=output_file,initial_file=initial_file, _EXTRA=ex2
...
for visit=0,12 do begin
    ...
    if visit eq 0 then begin
        ...
        r=replicate(fiducial_str, n_ext, 13)
