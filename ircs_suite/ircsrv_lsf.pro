function ircsrv_lsf, gh0_vec, gh1_vec=gh1_vec, oversamp=oversamp, first_pix=first_pix, npix_select=npix_select, verbose=verbose, decomp=decomp, neg_penalty=neg_penalty, neg_boost=neg_boost
...
if neg_penalty_switch eq 1 then begin
    abs_norm=total(abs(lsf_nonorm_com), 1, /double)
    diff_norm=total(lsf_nonorm_com, 1, /double)
